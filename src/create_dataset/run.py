...
import utils
from src.create_dataset import hdf5_getters
...
def run_parallel(func, my_iter):
    ...
    # results = list(tqdm(map(func, my_iter), total=len(my_iter)))
...
if use_pianoroll_dataset:
    midi_dataset_path = "../../data_files/lpd/lpd_5_full"
    extension = ".npz"
...
if os.path.exists(output_path) and not redo:
    ...
    if os.path.exists(output_path_incomplete):
        ...
        data_to_process = {key: value for key, value in data_to_process.items() if
                           key not in track_ids_already_processed}
        write_header = False

    with open(output_path_incomplete, "a", encoding="utf-8") as f_out:
...
if os.path.exists(output_path) and not redo:
    ...
    for k in match_scores_reversed.keys():
        match_scores_reversed[k] = collections.OrderedDict(
            sorted(match_scores_reversed[k].items(), reverse=True, key=lambda x: x[-1]))
...
if os.path.exists(output_path) and not redo:
    ...
    for pr in tqdm(midi_files_matched_unique):
        ...
        if trackid_to_spotify_features.get(sample_data['track_id'], None) is not None:
